Shader "Hidden/Shapes/Label Billboard"
{
	Properties
	{
		_SortingLayer ("Sorting Layer", Float) = 1158860747
	...
	SubShader
	{
		...
		Pass
		{
			...
			inline float4 Billboard(float4 vertex, float3 scale)
            {
                float4 scaledVertex = vertex;
                scaledVertex.xyz *= scale;
                return mul(UNITY_MATRIX_P,
                    mul(UNITY_MATRIX_MV, float4(0.0, 0.0, 0.0, 1.0))
                    + float4(scaledVertex.x, scaledVertex.y, 0.0, 0.0)
                    * float4(1, 1, 1.0, 1.0));
            }
			...
			v2f vert(appdata v)
			{
				...
				//float3 scale = float3(length(unity_ObjectToWorld[0].xyz), length(unity_ObjectToWorld[1].xyz), 1);
				float4 scale = UNITY_ACCESS_INSTANCED_PROP(CommonProps, _scale);
				o.vertex = Billboard(v.vertex, scale);
				o.uv = v.vertex.xy;
				return o;
			}
